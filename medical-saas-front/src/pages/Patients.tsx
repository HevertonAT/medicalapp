...
export default function Patients() {
  ...
  const calculateAge = (dataNascimento: string | undefined | null) => {
  ...
  // --- BUSCA OS PACIENTES COM TANSTACK QUERY ---
  const { data: patients = [], isLoading: loading } = useQuery<IPatient[]>({
    queryKey: ['patients'],
    queryFn: async () => {
  ...
  // --- QUERY DOS PRONTUÁRIOS (Laze loading) ---
  const { data: records = [], refetch: refetchRecords } = useQuery<IRecord[]>({
    queryKey: ['medical-records', currentPatient.id],
    queryFn: async () => {
      if (!currentPatient.id) return [];
      const res = await api.get(`/medical-records/patient/${currentPatient.id}`);
      return res.data;
    },
    enabled: false // Só busca quando abrir o modal de prontuário
  });
  ...
  const saveMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({ title: isEditing ? 'Atualizado!' : 'Criado!', status: 'success' });
      onClose();
      setCpfError('');
    },
    onError: (error: any) => {
  ...
  const openModal = (patient: IPatient | null = null) => {
      if (patient) {
          setCurrentPatient({
            id: patient.id,
  ...
  const handleOpenRecord = (patient: IPatient) => {
  ...
  const handlePrintEvolution = (record: IRecord) => {
