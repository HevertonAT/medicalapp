...
export default function Agenda() {
  ...
  const [minhasMacros, setMinhasMacros] = useState<any[]>([]); 
  ...
  const enforceDateLimit = (dateString: string) => {
  ...
  const { data: allClinics = [] } = useQuery<any[]>({
    ...
    enabled: currentUserRole === 'superuser'
  });

  const loading = loadingAppointments || loadingDoctors || loadingPatients;
  ...
  const calculateAge = (patientId: string | number) => {
  ...
  const startConsultationMutation = useMutation({
    mutationFn: async (appId: string) => {
      await api.patch(`/appointments/${appId}/status?novo_status=${STATUS_AGENDAMENTO.EM_ANDAMENTO}`);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['appointments'] });
    }
  ...
  const handleStartConsultation = async (app: any) => {
    ...
    try {
      if (app.status === STATUS_AGENDAMENTO.AGENDADO) {
          startConsultationMutation.mutate(app.id);
      }
  ...
  const finishConsultationMutation = useMutation({
    mutationFn: async (payload: any) => {
  ...
  const formatDoctorInfo = (name: string, specialtyArea: string, gender: string) => {
    ...
    const genderMap: Record<string, {m: string, f: string, d: string}> = {
  ...
  const openCancelModal = (app: any) => { setCurrentAppointment(app); setActionReason(''); onCancelOpen(); };
  
  const cancelMutation = useMutation({
    mutationFn: async (id: string) => {
        await api.patch(`/appointments/${id}/status?novo_status=${STATUS_AGENDAMENTO.CANCELADO}`);
    },
    onSuccess: () => {
  ...
  const handleConfirmCancel = () => {
    ...
    cancelMutation.mutate(currentAppointment.id);
  };
  ...
  const rescheduleMutation = useMutation({
    mutationFn: async (payload: any) => {
        await api.patch(`/appointments/${currentAppointment.id}/reschedule`, payload);
    },
    onSuccess: () => {
    ...
    onError: () => {
        toast({ title: 'Erro ao reagendar.', status: 'error' });
    }
  });
  ...
  const calendarEvents = filteredAppointments.map((app: any) => {
