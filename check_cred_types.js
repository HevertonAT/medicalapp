const url = "https://n8n-automations-production-0614.up.railway.app/api/v1/credential-types";
...
fetch(url, { headers: { "X-N8N-API-KEY": apiKey } })
  .then(res => res.json())
  .then(data => {
    const waTypes = data.data.filter(c => c.name.toLowerCase().includes('whatsapp'));
    console.log(JSON.stringify(waTypes.map(c => ({ name: c.name, displayName: c.displayName, properties: c.properties.map(p=>p.name) })), null, 2));
  })
